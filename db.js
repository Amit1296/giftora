...
async function getCoupons() {
  if (USE_PG) return pgGet("coupons", []);
  return readJson(COUPONS_FILE, []);
}

async function saveCoupons(coupons) {
  if (USE_PG) return pgSet("coupons", coupons);
  return writeJson(COUPONS_FILE, coupons);
}

/* ---------- Gift cards ---------- */
async function getGiftCards() {
  if (USE_PG) return pgGet("giftcards", []);
  return readJson(GIFTCARDS_FILE, []);
}

async function saveGiftCards(cards) {
  if (USE_PG) return pgSet("giftcards", cards);
  return writeJson(GIFTCARDS_FILE, cards);
}
...
async function getBanners() {
  if (USE_PG) return pgGet("banners", DEFAULT_BANNERS);
  return readJson(BANNERS_FILE, DEFAULT_BANNERS);
}

async function saveBanners(banners) {
  if (USE_PG) return pgSet("banners", banners);
